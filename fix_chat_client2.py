...
# Replace the specific handle_submit chunk
start_idx = text.find('  const handleSubmit = async (e: React.FormEvent) => {')
end_idx = text.find('  return (', start_idx)
...
start_effect_idx = text.find('  // Agent speaks first — onboarding message')
end_effect_idx = text.find('  useEffect(() => {', start_effect_idx + 10)

new_effect = '''  // Agent speaks first — onboarding message
  useEffect(() => {
...
if start_effect_idx != -1 and end_effect_idx != -1:
    text = text[:start_effect_idx] + new_effect + text[end_effect_idx:]

text = text.replace('                  onChange={e => setInput(e.target.value)}', '                  onChange={handleInputChange}')

text = text.replace('{m.modelUsed === "xiaomi/mimo-7b"', '{((m as any).modelUsed === "xiaomi/mimo-7b" || modelUsed === "xiaomi/mimo-7b")')
